...
Note: This is the command line version of the Java Spring backend project [here](https://github.com/54754N4/String-Obfuscator)
...
```
usage: strobf [-h] [-l MIN_OPS] [-u MAX_OPS] [-b MAX_BITS] -t LANG [-i INPUT | -f FILE | -s]

Obfuscates a string using a polymorphic engine into different languages. Generates a decryption/deobfuscation routine in any of the following target languages (which can be specified using the -t or
--target parameter): bash, c#, c_sharp, csharp, c, cpp, c++, javascript, js, java, masm64, powershell, ps, python, py
